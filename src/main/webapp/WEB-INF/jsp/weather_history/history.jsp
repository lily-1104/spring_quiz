		...
		<div class="contents d-flex">
			...
			<section class="weather-history col-10 mt-3 ml-5">
				...
				<table class="table text-center mt-4">
					...
					<tbody>
						<c:forEach items="${weatherHistoryList }" var="weather">	<%-- controller의 "weatherHistoryList" --%>
						<tr>
							<td><fmt:formatDate value="${weather.date }" pattern="yyyy년 M월 d일" /></td>
							<td>	
								<c:choose>		<%-- 날씨는 4개 중 하나라서 choose 태그 사용 --%>
									<c:when test="${weather.weather eq '맑음' }">
										<img src="/img/sunny.jpg" alt="맑음">
									</c:when>
									<c:when test="${weather.weather == '구름 조금' }">
									...
									<c:otherwise>
										${weather.weather }		<%-- 위의 4개 외에 다른 날씨는 그림이 아닌 글자로 나오게 함 --%>
									</c:otherwise>
								</c:choose>
							</td>
