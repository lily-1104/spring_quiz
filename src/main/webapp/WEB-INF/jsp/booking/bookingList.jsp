...
<body>
	...
	<script>
		...
		$(document).ready(function() {
			...
			$('.del-btn').on('click', function() {	// btn이 클래스라서 '.del-btn' => 앞에 . 붙여야함
				
				// alert('클릭');
			
				let id = $(this).date('booking-id'); 	// data-booking-id 의 booking-id
				// alert(id);
				...
				$.ajax({
					...
					// request
					type:"delete"	// delete는 post의 일종
					, url:"/booking/delete-booking"
					, data:{"id":id}
					
					// response
					, success:function(data) {	// JSON => dictionary 형태로 변환
					...
					, error:function(request, status, error) {	// 서버 에러
